...
import vn.com.onesoft.livetube.io.message.core.MessageBuffer;
...
public class ConnectionManager implements Runnable {
	...
	public void run() {
		...
		while (true) {
			if (isOnline
					&& (System.currentTimeMillis() - lastPingReceivedTime) < 20000) {
				try {
	...
	public boolean write(BaseMessage mOut) {
		...
		Log.d("BigFox", String.format("[SEND] %s\n%s",
				"".equals(m.name()) ? mOut.getClass().toString() : m.name(),
				BigFox.toString(mOut)));
		curMSequence++;
