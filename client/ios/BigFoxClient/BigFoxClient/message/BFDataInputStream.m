...
@implementation BFDataInputStream
...
-(id) initWithBytes:(char *)aData :(int)length {
    self = [self init];
    if (self != nil) {
        data = [[NSData alloc] initWithBytes:aData length:length];
    }
    return self;
}
...
+ (id)dataInputStreamWithData:(NSData *)aData {
    BFDataInputStream *dataInputStream = [[self alloc] initWithData:aData];
